...
def debug_input_vis(self, imgs, targets, ids):
    ...
    for i in range(n):
        ...
        if self.config.training_mosaic:
            img_id = self.train_loader.dataset._dataset.id_to_img_map[ids[i]]
        else:
            img_id = self.train_loader.dataset.id_to_img_map[ids[i]]
