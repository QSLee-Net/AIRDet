...
def postprocess_gfocal(prediction, num_classes, conf_thre=0.7, nms_thre=0.45, imgs=None):
    ...
    for i, image_pred in enumerate(prediction):
        ...
        if nms_type == 'yolox':
            ...
            detections, scores, labels = multiclass_nms(multi_bboxes, multi_scores, conf_thre, nms_thre, 100)
...
def postprocess(prediction, num_classes, conf_thre=0.7, nms_thre=0.45, imgs=None):
    box_corner = prediction.new(prediction.shape)
    box_corner[:, :, 0] = prediction[:, :, 0] - prediction[:, :, 2] / 2
    box_corner[:, :, 1] = prediction[:, :, 1] - prediction[:, :, 3] / 2
    box_corner[:, :, 2] = prediction[:, :, 0] + prediction[:, :, 2] / 2
