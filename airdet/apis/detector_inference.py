...
def compute_on_dataset(config, model, data_loader, device, timer=None, tta=False):
    model.eval()
    results_dict = {}
    ...
    for _, batch in enumerate(tqdm(data_loader)):
        ...
        with torch.no_grad():
            if timer:
                timer.tic()
            if config.testing.use_tta:
                output = im_detect_bbox_aug(model, images, device, config)
            else:
...
def inference(
    ...
    total_timer.tic()
    predictions = compute_on_dataset(config, model, data_loader, device, inference_timer)
