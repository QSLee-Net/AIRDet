...
from airdet.utils.boxes import filter_results
...
def im_detect_bbox_aug(model, images, device, config):
    ...
    for scale in config.testing.augmentation.scales:
        ...
        boxlists_scl = im_detect_bbox_scale(
            model, images, scale, max_size, device, config
        )
    ...
    for boxlist in boxlists:
        results.append(filter_results(boxlist, config.model.head.num_classes, config.testing.augmentation.nms_thres))
...
def im_detect_bbox(model, images, target_scale, target_max_size, device, config):
    ...
    transform = T.Compose([
        ...
        T.ToTensor(),
        T.Normalize(
            mean=config.dataset.input_pixel_mean, std=config.dataset.input_pixel_std, to_bgr255=config.dataset.input_to_bgr255
        )
    ])
...
def im_detect_bbox_scale(model, images, target_scale, target_max_size, device, config, hflip=False):
    ...
    """
    if hflip:
        boxlists_scl = im_detect_bbox_hflip(model, images, target_scale, target_max_size, device, config)
    else:
